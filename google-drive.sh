...
echo "Имя папки:"
read folder
echo "Создаем папку Google Drive ($HOME/$folder)"
mkdir -p "$HOME/$folder"
echo "Имя пользователя (с сайта http://dav-pocket.appspot.com/):"
read username
echo "Пароль:"
read -s password
...
if sudo grep -Fxq "https://dav-pocket.appspot.com/docso $HOME/$folder davfs noauto,user 0 0" /etc/fstab
then
	echo "Такая запись уже есть в /etc/fstab"
else
	echo "Добавляем запись в /etc/fstab"
	echo "https://dav-pocket.appspot.com/docso $HOME/$folder davfs noauto,user 0 0" | sudo tee -a /etc/fstab >> /dev/null
fi
echo "Теперь нужна перезагрузка"
echo "Монтировать диск Google Drive можно командой mount $HOME/$folder"
